                ...
                <table class="divide-y divide-gray-300 ">
                    ...
                    @foreach($user->packages as $package)
                        <tr class="whitespace-nowrap">
                            ...
                            <td class="px-12 py-4">
                                <div class="text-sm text-gray-500"> {{ $package->pivot->start_date }}</div>
                            </td>

                            <td class="px-12 text-sm text-gray-500">
                                {{ $package->pivot->start_date }}
                            </td>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                @endif
                    ...
                    @if(session()->has('error'))
                        <div class="px-4 py-3 leading-normal text-red-700 bg-blue-100 rounded-lg" role="alert">
                            {{ session()->get('error') }}
                        </div>
                    @endif
                ...
                @endif
                  ...
                  @if($user->enabled)
                    <form class="space-y-4 text-gray-700 space-x-2" action="{{ route('user-packages.store', $user) }}" method="POST">
                        @csrf
                    ...
                    @else
                        <div class="px-4 py-3 leading-normal text-red-700 bg-blue-100 rounded-lg" role="alert">
                            To assign packages please enable the status of user
                        </div>
                    @endif
